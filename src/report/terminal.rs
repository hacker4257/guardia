...
pub fn print_report(findings: &[Finding], duration: Duration) {
    ...
    for (i, finding) in findings.iter().enumerate() {
        print_finding(i + 1, finding);
    }

    print_summary(findings, duration);
}

fn print_finding(index: usize, finding: &Finding) {
...
fn print_summary(findings: &[Finding], duration: Duration) {
